...
$(window).scroll(function () {
  ...
  //nav.active
    if(winTop >= $('.main_visual').offset().top) {
      $('.main_nav a').removeClass('active')
      $('.main_nav a').eq(0).addClass('active')
    }
    if(winTop >= $('.earth_wrap').offset().top) {
      $('.main_nav a').removeClass('active')
      $('.main_nav a').eq(1).addClass('active')
    }
    if(winTop >= 2200) {
      $('.main_nav a').removeClass('active')
      $('.main_nav a').eq(2).addClass('active')
    }
    if(winTop >= 2980) {
      $('.main_nav a').removeClass('active')
      $('.main_nav a').eq(3).addClass('active')
    }
    if(winTop >= 3980) {
      $('.main_nav a').removeClass('active')
      $('.main_nav a').eq(4).addClass('active')
    }

}) */ //scroll()

// dep1의 li에 마우스를 올렸을 때, dep1의 a만 색상 변경
$('.allmenu_right .dep1>li').hover(
  function() {
    // dep1의 a 태그만 main-color로 변경
    $('this').css({
      'color': 'var(--main-color)',
    });
  },
  function() {
    // 마우스를 벗어났을 때 다시 기본 색상으로 복원
    $(this).css({
      'color': '',
    });
  }
...
// dep3의 li의 a에 마우스를 올렸을 때, 해당 a만 색상 변경
$('.allmenu_right .dep3 li a').hover(
  function() {
    // dep3의 a에만 main-color와 underline 적용
    $(this).css({
      'color': 'var(--main-color)',
